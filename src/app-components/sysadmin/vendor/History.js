...
class History extends Component {
  ...
  /*UNSAFE_componentWillUpdate(nextProps) {
    if (nextProps.vendors.loading === false && this.state.init) {
      console.log(" sync vendors props with state  ...");
      this.Paginate(
        [...nextProps.vendors.payload],
        this.state.totalRowsPerPage
      );
    }
    if (
      nextProps.vendors.loading === true &&
      this.props.vendors.loading === false
    ) {
  ...
  render() {
    const vendors = this.props.vendors.loading ? (
      <tr>
        <td colSpan="9">
          <SkeletonLoader />
        </td>
    ...
    return (
      <>
        <Card className="card-box shadow-none d-none d-md-block">
          ...
          {!this.props.vendors.loading && this.state.current.length !== 0 && (
