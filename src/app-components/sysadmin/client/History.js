...
class History extends Component {
  ...
  /*UNSAFE_componentWillUpdate(nextProps) {
    if (nextProps.clients.loading === false && this.state.init) {
      console.log(" sync clients props with state  ...");
      this.Paginate(
        [...nextProps.clients.payload],
        this.state.totalRowsPerPage
      );
    }
    if (
      nextProps.clients.loading === true &&
      this.props.clients.loading === false
    ) {
  ...
  render() {
    const clients = this.props.clients.loading ? (
      <tr>
        <td colSpan="9">
          <SkeletonLoader />
        </td>
      ...
      <>
        <Card className="card-box shadow-none d-none d-md-block">
          <div className="px-4 pt-4 text-primary">
            <h5 className="font-weight-bold text-dark">Listes des clients</h5>
          </div>
          ...
          {!this.props.clients.loading && this.state.current.length !== 0 && (
